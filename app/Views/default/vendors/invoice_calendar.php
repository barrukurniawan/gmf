...
if($user_info['user_type'] == 'staff'){
	$completed_invoices = $InvoicesModel->where('company_id',$user_info['company_id'])->where('status', 1)->orderBy('invoice_id', 'ASC')->findAll();
	$pending_invoices = $InvoicesModel->where('company_id',$user_info['company_id'])->where('status', 0)->orderBy('invoice_id', 'ASC')->findAll();
} else {
	$completed_invoices = $InvoicesModel->where('company_id',$usession['sup_user_id'])->where('status', 1)->orderBy('invoice_id', 'ASC')->findAll();
	$pending_invoices = $InvoicesModel->where('company_id',$usession['sup_user_id'])->where('status', 0)->orderBy('invoice_id', 'ASC')->findAll();
}
	...
	$('#calendar_hr').fullCalendar({
		...
		events: [
			<?php foreach($completed_invoices as $cinvoice):?>
			{
				...
				urllink: '<?php echo site_url().'erp/view-leave-info/'.uencode($cinvoice['invoice_id']);?>',
			...
			<?php foreach($pending_invoices as $pinvoice):?>
			{
				...
				urllink: '<?php echo site_url().'erp/view-leave-info/'.uencode($pinvoice['invoice_id']);?>',
